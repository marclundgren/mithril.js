Mithril = m = new function app(window, undefined) {
  ...
  function jsonp(options) {
    var urlCallbackVal = getParams(options.url).callbackKey;

    var callbackName = urlCallbackVal ? urlCallbackVal : "mithril_callback_" + new Date().getTime() + "_" + (Math.round(Math.random() * 1e16)).toString(36)
  ...
  m.request = function(xhrOptions) {
    ...
    if (xhrOptions.dataType && xhrOptions.dataType.toLowerCase() === "jsonp") {
      jsonp(xhrOptions)
    }
    else {
      ajax(xhrOptions)
